...
class Db_PDO extends DbObject{
	...
	function __connect($forceReconnect=false){
		if(empty(Db_PDO::$globals[$this->key]) || $forceReconnect){
			...
			try{
			Db_PDO::$globals[$this->key] = new PDO($this->prefix.":dbname=".SAE_MYSQL_DB.";host=".SAE_MYSQL_HOST_M.";port=".SAE_MYSQL_PORT,SAE_MYSQL_USER,SAE_MYSQL_PASS);
			}catch(Exception $e){
				if(defined("DEBUG")){
